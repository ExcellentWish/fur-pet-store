...
class ProductDetailReview(View):
    def get(self, request, product_id, *args, **kwargs):
        ...
        reviews = Review.objects.filter(
            product=product, approved=True).order_by(
            "-created_date"
        )
    ...
    def post(self, request, product_id, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            review.save()
            messages.success(
                request, "You left a review. Please wait for approval")
            return HttpResponseRedirect(
                reverse("product_detail_review", args=[product_id])
            )

        """ If the form is not valid,
            return to the template with the errors
             and the original form data"""
        context = {
            "product": product,
            "reviews": reviews,
            "reviewed": True,
